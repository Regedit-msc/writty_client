...
const Search = () => {
    ...
    function handleKeyPress(e) {
        if (e.key === "Enter") {
            const inputValueToUrl = new URLSearchParams({ [searchQuery]: inputValue }).toString();
    ...
    if (searchTerm.startsWith("user")) {
        return <>
            <div className="search_heading">  Search results for  <span style={{ marginRight: "0.7rem", marginLeft: "0.7rem" }} >{searchString}</span>.</div>
            <div className="search_results">
                {
                    searchData?.length > 0 ? searchData.map((result, index) => {
                        // console.log(refs);
                        return <div className="fade_in_results " key={index}>
                            ...
                            <div className="search_result_user">
                                ...
                                }} alt="search_restult" src={result?.profileImageUrl ?? PlaceholderProfileImage} />
                        ...
                        ><div className="search_notfound" >
                            <code className="prettyprint" style={{ fontSize: "2rem" }}  >No results found for {searchString} .</code>
    ...
    return <>
        <div className="search_search_container">
            ...
            <input type="search" name="search" className="search_search" placeholder="Search anything e.g John Doe, binary search python..." autoComplete="off" onChange={handleSearchInputChange} onKeyPress={handleKeyPress} ref={inputRef} />

        </div>
        <div className="search_query_btns_container">
            <div className={`search_query_button ${searchQuery === "user" ? "active_search_query_btn" : ""}`} onClick={() => {
                setSearchQuery("user");
            }} > User</div>
            <div className={`search_query_button ${searchQuery === "code" ? "active_search_query_btn" : ""}`} onClick={() => {
                setSearchQuery("code");
            }} > Code</div>
            <div className={`search_query_button ${searchQuery === "post" ? "active_search_query_btn" : ""}`} onClick={() => {
                setSearchQuery("post");
            }} > Post</div>
        </div>
        <div className="search_info" >
            <span>Press   <code><i>Enter</i></code> to <h1>search</h1> </span>
            <span>Press    <code><i>F</i></code> to <h1>focus</h1> </span>
        </div>
    </>
}
